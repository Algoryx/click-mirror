...
SCENARIO("sensormessage serialization from file", "[click]")
{
    GIVEN("A serialized sensormessage")
    {
        ...
        std::fstream input(TESTDATA_DIR + "sensormessage.bin", std::ios::in | std::ios::binary);
        REQUIRE(input);

        WHEN("creating sensormessage from bytes generated by python")
        {
            MessageSerializer serializer;
            std::unique_ptr<Message> message = serializer.sensorMessageFromIStream(&input);
            REQUIRE(message->messageType() == MessageType::SensorMessageType);
            std::unique_ptr<SensorMessage> sensorMessage = toSensorMessage(std::move(message));
            ...
            {
                MessageSerializer serializer;
                std::string bytes = serializer.serializeToString(*sensorMessage);
                std::unique_ptr<Message> message = serializer.fromBytes(bytes);
